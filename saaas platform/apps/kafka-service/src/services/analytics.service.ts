...
export const updateUserAna;ytics=async(event:any)=> {
    try{
   ...
   //always store product_view for recommmendation

    if(event.action==="product_view" || !actionExists){
        updatedActions.push({
    ...
    //keep only the last 100 actions to limit data size
    if(updatedActions.length>100){
        updatedActions.shift();
    }

       const extraFields=Records<String,any>={};
     ...
     try{
      ...
      if(event.action==="product_view") updateFields.views={increment:1};
        if(event.action==="add_to_cart")
             updateFields.addToCartCount={increment:1};
        if(event.action==="add_to_wishlist")
             updateFields.addToWishlistCount={increment:1};
        if(event.action==="remove_from_cart")
             updateFields.addToCartCount={decrement:1}; 
        if(event.action==="remove_from_wishlist")
             updateFields.addToWishlistCount={decrement:1};
