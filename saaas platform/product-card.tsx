...
const ProductCard = ({
    ...
    const deviceIinfo=useDeviceTracking();
    const addToCart =useStore((state:any => state.addToCart));
    const addToWishlist =useStore((state:any => state.addToWishlist));
    const removeFromWishlist =useStore((state:any => state.removeFromWishlist));
    const wislist=useStore((state:any) => state.wishlist);
    const isWishlisted=wislist.some((item:any) => item.id === product.id);
    const cart=useStore((state:any) => state.cart);
    const isInCart=cart.some((item:any) => item.id === product.id);
       
       useEffect(() => {
            if (isEvent && product?.event_end_time) {
          ...
          <div className="flex items-center gap-2">
           ...
           <span className="text-sm line-through text-gray-400">
              ...
              <span className="text-green-500 text-sm font-medium">
                ...
                {isEvent && timeLeft && (
                    <div className="mt-2">
                        <span className="inline-block text-xs bg-orange-100 text-orange-800 px-2 py-1 rounded">
                            {timeLeft}
                        </span>
                     </div>
                  )}
